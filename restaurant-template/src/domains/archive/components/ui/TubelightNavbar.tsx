...
interface NavBarProps { items: NavItem[]; className?: string }
...
export function NavBar({ items, className }: NavBarProps) {
  ...
  return (
    <div className={cn("fixed left-1/2 z-50 -translate-x-1/2 sm:pt-6", isMobile ? "bottom-0 mb-6" : "top-0", className)}>
      <div className="flex items-center gap-3 rounded-full border border-border bg-background/5 px-1 py-1 backdrop-blur-lg shadow-lg">
        {items.map((item) => {
