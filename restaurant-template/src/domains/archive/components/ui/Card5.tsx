...
export const HighlightCard = React.forwardRef<HTMLDivElement, HighlightCardProps>(
  (
    { title, description, metricValue, metricLabel, buttonText, onButtonClick, icon, color = "default", className },
    ref
  ) => {
    ...
    return (
      <motion.div
        ...
        style={{
          ...
          ['--card-from-color' as any]: `hsl(${theme.from})`,
          ['--card-to-color' as any]: `hsl(${theme.to})`,
          ['--card-foreground-color' as any]: `hsl(${theme.foreground})`,
          // Apply colors and dot pattern
          color: 'var(--card-foreground-color)' as any,
          backgroundImage: `
      ...
      >
        ...
        <div className="absolute top-0 right-6 h-16 w-12 bg-white/95 backdrop-blur-sm [clip-path:polygon(0%_0%,_100%_0%,_100%_100%,_50%_75%,_0%_100%)] dark:bg-zinc-800/80">
          <div 
            ...
            style={{ color: 'var(--card-from-color)' as any }}
          >
            {icon}
          </div>
        </div>
